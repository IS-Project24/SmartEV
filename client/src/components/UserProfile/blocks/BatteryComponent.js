...
const BatteryComponent = () => {
  const [battery, setBattery] = useState(null);
  const [charging, setCharging] = useState(false); // Hard-coded value for charging
  const [level, setLevel] = useState(20); // Hard-coded value for battery level
  const [statusText, setStatusText] = useState(
    'Charging... <i class="ri-flashlight-line animated-green"></i>'
  );
  const [batteryLiquidHeight, setBatteryLiquidHeight] = useState("50%");
  const [batteryLiquidClass, setBatteryLiquidClass] = useState("gradient-color-yellow");

  useEffect(() => {
    // Update the battery status when the component mounts
    updateBatteryStatus(level / 100, charging);
  }, []);
